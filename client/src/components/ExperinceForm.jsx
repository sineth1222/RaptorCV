...
const ExperinceForm = ({data, onChange}) => {
    ...
    const generateDescription = async (index) => {
        ...
        const promt = `enhance this job description ${experience.description} for the position of ${experience.position} at ${experience.company}.`

        try {
            const { data } = await api.post('api/ai/enhance-job-desc', {userContent: promt}, {headers: {Authorization: token}})
  ...
  return (
    <div className='space-y-6'>
            <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-8 sm:gap-0'>
                ...
                <button onClick={addExperience} className='flex items-center w-fit ml-auto sm:ml-0 gap-2 px-3 py-2 text-sm bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors'>
