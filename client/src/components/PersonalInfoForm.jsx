...
const PersonalInfoForm = ({data, onChange, removeBackground, setRemoveBackground}) => {
    ...
    const haddleChange = (field, value) => {
        onChange({
            ...data,
            [field]: value
        })
    ...
    <div>
      ...
      <div className='flex items-center gap-2'>
        <label>
            ...
            <input type="file" accept='image/jpeg, image/png, image/jpg' className='hidden' onChange={(e) => haddleChange('image', e.target.files[0])}/>
        </label>
      ...
      {fields.map ((field)=> {
        ...
        return (
            <div key={field.key} className='space-y-1 mt-5'>
                ...
                <input type={field.type} value={data[field.key] || ""} onChange={(e)=>haddleChange(field.key, e.target.value)} className='mt-1 w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring focus:ring-blue-500 outline-none transition-colors text-sm' placeholder={`Enter your ${field.label.toLowerCase()}`} required={field.required} />
            </div>
        )
      })}
